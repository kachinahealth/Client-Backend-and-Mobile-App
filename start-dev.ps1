...
if (!(Test-Path $envFile)) {
    ...
    Write-Host "A .env file should exist in the backend folder with placeholder values." -ForegroundColor Red
    Write-Host "Please check your project setup and ensure the .env file exists." -ForegroundColor Red
    exit 1
} else {
    Write-Host "✅ .env file found in backend directory" -ForegroundColor Green
    Write-Host "⚠️  Reminder: Make sure your .env file contains your actual Supabase credentials!" -ForegroundColor Yellow
    Write-Host ""
}
