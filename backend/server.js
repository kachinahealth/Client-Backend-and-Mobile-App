...
app.get('/api/users', authenticateToken, async (req, res) => {
  try {
    ...
    if (clinicalTrialId && userProfile.role === 'admin') {
      ...
      const assignedUserIds = assignmentData.map(assignment => assignment.user_id);
...
app.get('/api/news', authenticateToken, async (req, res) => {
  try {
    ...
    // Filter by specific clinical trial if requested
    let queryTrialIds = accessibleTrialIds;
    if (clinicalTrialId && accessibleTrialIds.includes(clinicalTrialId)) {
      console.log(`🔍 Filtering news by specific clinical trial: ${clinicalTrialId}`);
      queryTrialIds = [clinicalTrialId];
    }

    console.log('🎯 Query trial IDs:', queryTrialIds);

    if (queryTrialIds.length === 0) {
    ...
    const { data, error } = await supabaseAdmin
      ...
      `)
      .in('clinical_trial_id', queryTrialIds)
      .order('published_at', { ascending: false });
...
app.post('/api/news', authenticateToken, async (req, res) => {
  try {
    const { title, content, description, clinical_trial_id } = req.body;
...
app.get('/api/hospitals', authenticateToken, async (req, res) => {
  try {
    ...
    // Filter by clinical trial if specified
    if (clinicalTrialId && userProfile.role === 'admin') {
      console.log(`🔍 Filtering hospitals by clinical trial: ${clinicalTrialId}`);
      query = query.eq('clinical_trial_id', clinicalTrialId);
    } else {
      // Default behavior - filter by organization
      query = query.eq('organization_id', userProfile.organization_id);
    }

    const { data, error } = await query.order('randomized_patients', { ascending: false });
    ...
    if (data && data.length > 0) {
      ...
      console.log('No hospitals data found in database');
...
app.get('/api/training-materials', authenticateToken, async (req, res) => {
  try {
    ...
    // Filter by clinical trial if specified (admin only)
    if (clinicalTrialId && userProfile.role === 'admin') {
      console.log(`🔍 Filtering training materials by clinical trial: ${clinicalTrialId}`);
      query = query.eq('clinical_trial_id', clinicalTrialId);
    }
...
app.get('/api/study-protocols', authenticateToken, async (req, res) => {
  try {
    ...
    let queryTrialIds = accessibleTrialIds;
    if (clinicalTrialId && accessibleTrialIds.includes(clinicalTrialId)) {
      console.log(`🔍 Filtering study protocols by specific clinical trial: ${clinicalTrialId}`);
      queryTrialIds = [clinicalTrialId];
    }
    ...
    if (!error && data && data.length > 0) {
      ...
      // No study protocols found
      console.log('📨 No study protocols found in database');
      studyProtocols = [];
    }
...
app.get('/api/analytics', authenticateToken, async (req, res) => {
  try {
    ...
    // Mock data for testing (filtered by clinical trial if specified)
    let mockAnalytics = [
      {
        id: '1',
        user_id: 'user1',
    ...
    // Filter by clinical trial if specified (admin only)
    if (clinicalTrialId && userProfile.role === 'admin') {
      console.log(`🔍 Filtering analytics by clinical trial: ${clinicalTrialId}`);
      // For mock data, we could add clinical_trial_id to each record and filter
      // For now, we'll return all data since this is mock data
      // In a real implementation, you'd filter based on clinical trial assignments
    }
